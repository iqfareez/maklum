## Deploy app using Cloudpanel on a VPS
...
1. Run few scripts to set up the environment.
    ...
    ```
1. Update your `.env` file with the database configuration _(Depending on how to setup the Cloudpanel in the first place, you may have choose the MariaDB or MySQL database. So update the `DB_CONNECTION` accordingly)_:
    ```bash
    DB_CONNECTION=mysql
    DB_HOST=127.0.0.1
    DB_PORT=3306
